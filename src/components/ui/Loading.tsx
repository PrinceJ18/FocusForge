...
export function CardSkeleton({ className = '' }: { className?: string }) {
  return (
    <div className={`p-6 rounded-2xl border border-border bg-white/[0.02] flex flex-col space-y-4 ${className}`}>
...
export function ListSkeleton({ count = 3, className = '' }: { count?: number; className?: string }) {
  return (
    <div className={`space-y-3 ${className}`}>
      {[...Array(count)].map((_, i) => (
        <div key={i} className="flex items-center p-4 rounded-xl border border-border bg-white/[0.02]">
...
export function TableSkeleton({ rows = 5, columns = 4, className = '' }: { rows?: number; columns?: number; className?: string }) {
  return (
    <div className={`w-full overflow-hidden rounded-2xl border border-border ${className}`}>
      <div className="bg-white/[0.03] p-4 border-b border-border flex">
...
export function LeaderboardSkeleton() {
  return (
    <div className="space-y-6">
      {/* Top Banner Skeleton */}
      <div className="rounded-2xl border border-border bg-background-card-hover p-6 flex flex-col md:flex-row items-center gap-6">
