-- Task Management V2: Add status column and migrate data
...
-- 3. Drop old completed column
ALTER TABLE public.tasks 
DROP COLUMN IF EXISTS completed;

NOTIFY pgrst, 'reload schema';

-- 4. Update task_completions table to use status
ALTER TABLE public.task_completions 
ADD COLUMN IF NOT EXISTS status text NOT NULL DEFAULT 'completed' 
CHECK (status IN ('pending', 'completed', 'wont_do'));

-- 5. Migrate task_completions completed boolean to status
...
-- 6. Update complete_task RPC to use status
...
BEGIN
  ...
  IF v_is_recurring THEN
    ...
    v_completed_at := now();
    INSERT INTO public.task_completions (user_id, task_id, occurrence_date, status, completed_at, created_at, updated_at)
    VALUES (v_user_id, p_task_id, p_occurrence_date, 'completed', v_completed_at, now(), now());
  ...
  ELSE
    ...
    v_completed_at := now();
    UPDATE public.tasks 
    SET status = 'completed', completed_at = v_completed_at, updated_at = now()
    WHERE id = p_task_id;
  END IF;
...
-- 7. Update uncomplete_task RPC to use status
...
BEGIN
  ...
  ELSE
    ...
    IF v_was_completed THEN
      UPDATE public.tasks SET status = 'pending', completed_at = NULL, updated_at = now() WHERE id = p_task_id;
    END IF;
  END IF;
...
-- 8. Create mark_task_wont_do RPC
...
BEGIN
  ...
  IF v_is_recurring THEN
    ...
    DELETE FROM public.task_completions WHERE task_id = p_task_id AND occurrence_date = p_occurrence_date AND user_id = v_user_id;
    INSERT INTO public.task_completions (user_id, task_id, occurrence_date, status, completed_at, created_at, updated_at)
    VALUES (v_user_id, p_task_id, p_occurrence_date, 'wont_do', v_completed_at, now(), now());
  ELSE
    UPDATE public.tasks SET status = 'wont_do', completed_at = v_completed_at, updated_at = now() WHERE id = p_task_id;
  END IF;
